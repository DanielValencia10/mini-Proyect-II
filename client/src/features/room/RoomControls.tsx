...
export function RoomControls({
    ...
    const isMicBlocked = micPermission === 'denied' || micPermission === 'unavailable'
    const isCamBlocked = camPermission === 'denied' || camPermission === 'unavailable'
    ...
    return (
        <footer className="shrink-0 bg-gray-900 border-t border-gray-800 px-4 sm:px-6 py-3 sm:py-4 flex items-center justify-center gap-3 sm:gap-4">
            <button
                ...
                title={isMicBlocked ? 'Micrófono bloqueado - Haz clic para ver cómo habilitarlo' : micOn ? 'Desactivar micrófono' : 'Activar micrófono'}
            >
            ...
            <button
                ...
                title={isCamBlocked ? 'Cámara bloqueada - Haz clic para ver cómo habilitarla' : camOn ? 'Desactivar cámara' : 'Activar cámara'}
            >
            ...
            <button
                ...
                className={`${base} ${
                    isScreenSharing
                        ? 'bg-cyan-600 hover:bg-cyan-500 text-white ring-2 ring-cyan-400/50'
                        : 'bg-gray-700 hover:bg-gray-600 text-white'
                }`}
            >
            ...
            <button onClick={onToggleChat} className={`${base} ${chatOpen ? 'bg-cyan-600 hover:bg-cyan-500' : 'bg-gray-700 hover:bg-gray-600 text-white'}`}>
                <MessageSquare className="h-5 w-5" />
            </button>

            <button onClick={onLeave} className={`${base} bg-red-600 hover:bg-red-500 shadow-[0_0_10px_rgba(239,68,68,0.4)] text-white`}>
